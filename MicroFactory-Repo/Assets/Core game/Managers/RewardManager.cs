...
public class RewardManager : MonoBehaviour // reward manager
{
    ...
    public NodeData[] DifficultyPoolAdjust(NodeData[] nodesPool, int actualCicle)
    {
        ...
        if (actualCicle <= 3)
        {
            return nodesPool.Where(n => n.tags.Contains("Producer") || UnityEngine.Random.Range(0, 1f) > 0.5f).ToArray();
        }
        else if (actualCicle <= 9)
        {
            return nodesPool.Where(n => n.tags.Contains("Transformer") || UnityEngine.Random.Range(0, 1f) > 0.5f).ToArray();
        }
        else if (actualCicle <= 12)
        {
            return nodesPool.Where(n => n.tags.Contains("Seller") || UnityEngine.Random.Range(0, 1f) > 0.5f).ToArray();
        }
